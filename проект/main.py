    ...
    if db:
        ...
        if len(interactions) < 3:  # Мало данных для персонализации
            ...
            await message.answer(
                ...
                "Я уже изучил ваши предпочтения и готов "
                "рекомендовать анекдоты по душе! 🎭\n\n"
                "Нажмите 🎲 Новый анекдот для "
                "персонализированной рекомендации!",
                reply_markup=get_main_keyboard(),
            )
    else:
        await message.answer(
...
async def profile_command(message: types.Message):
    ...
    if profile["favorite_theme"]:
        fav_theme = next(
            t for t in profile["themes"]
            if t["id"] == profile["favorite_theme"]
        )
        profile_text += (
            f"\n🎯 **Чаще всего рекомендую:** "
            f"{fav_theme['emoji']} {fav_theme['name']}"
        )

    profile_text += (
        "\n\n✨ Продолжайте оценивать анекдоты "
...
async def send_personalized_joke(message: types.Message):
    ...
    if joke:
        ...
        if "theme_name" in joke and "theme_emoji" in joke:
            theme_info = (f"\n\n🔖 Тема: "
                          f"{joke['theme_emoji']} {joke['theme_name']}")
...
<<<<<<< HEAD
        "🐈 Животные,🎓 Школьные,💻 Работа,🔞 Черный юмор, 🎭 Разное\n\n"
=======
        "💻 Программисты, 🎓 Студенты, 💼 Работа, "
        "👨‍👩‍👧‍👦 Семья, 🎭 Разное\n\n"
>>>>>>> 83be37faec4866d9cc0e84cd43c26c4ba7e0fff0
        "📝 **Требования:**\n"
        "• Минимум 10 символов\n"
...
async def add_joke_text(message: types.Message, state: FSMContext):
    ...
    if len(text) < 10:
        await message.answer(
            "❌ Анекдот слишком короткий. Минимум 10 символов."
        )
        return

    if len(text) > 1000:
        await message.answer(
            "❌ Анекдот слишком длинный. Максимум 1000 символов."
        )
        return
    ...
    try:
        ...
        for theme_id in themes:
            with get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(
                    "SELECT name, emoji FROM themes WHERE id = ?",
                    (theme_id,)
                )
                row = cursor.fetchone()
                if row:
...
async def add_joke_confirmation(message: types.Message, state: FSMContext):
    ...
    if message.text.lower() in positive_responses:
        ...
        if not joke_text:
            await state.finish()
            await message.answer(
                "❌ Ошибка: текст анекдота не найден.",
                reply_markup=get_main_keyboard()
            )
            return
        ...
        if db:
            ...
            if liked:
                await callback_query.message.answer(
                    ...
                    "что вам нравятся такие анекдоты!"
                )
            else:
                await callback_query.message.answer(
